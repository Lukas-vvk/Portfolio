...
$sql_comments = "SELECT * FROM user_comments WHERE approved = 0";
$result_comments = $conn->query($sql_comments);

$sql_replies = "SELECT * FROM user_replies WHERE approved = 0";
$result_replies = $conn->query($sql_replies);

if (!$result_comments || !$result_replies) {
    die("Error fetching comments or replies: " . $conn->error);
}
?>
<!DOCTYPE html>
<html lang="lt">
<head>
    ...
    <title>Komentarų administravimas</title>

    <link rel="stylesheet" href="header_nav_style.css">
    <link rel="stylesheet" href="admin_comments.css">
...
<div class="comment-administration">
    <h2 class="heading">Komentarų administravimas</h2>
        ...
        <table class="comment-table">
            ...
            if ($result_comments->num_rows > 0) {
                while($row = $result_comments->fetch_assoc()) {
                    ...
                    echo "<td>";
                    echo "<a href='approve_comment.php?id=" . $row['id'] . "' class='action-link'>Patvirtinti</a> | ";
                    echo "<a href='admin_delete_comment.php?id=" . $row['id'] . "' class='action-link'>Trinti</a>";
                    echo "</td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='2' class='no-comment'>Nėra laukiančių komentarų</td></tr>";
            }
            ?>
        </table>
    </div>
...
<div class="comment-administration">
<h2 class="heading">Atsakymų administravimas</h2>
<div class="comment-table-container">
    <table class="comment-table">
        <tr class="table-header">
            <th>Atsakymas</th>
        ...
        if ($result_replies->num_rows > 0) {
            while($row = $result_replies->fetch_assoc()) {
                ...
                echo "<td>";
                echo "<a href='approve_reply.php?id=" . $row['id'] . "' class='action-link'>Patvirtinti</a> | ";
                echo "<a href='admin_delete_reply.php?id=" . $row['id'] . "' class='action-link'>Trinti</a>";
                echo "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='2' class='no-comment'>Nėra laukiančių atsakymų</td></tr>";
        }
        ?>
    </table>
    </div>
